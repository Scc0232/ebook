...
<script>
	(function() {
		...
		var dgOption = {
			...
			// 工具栏
			toolbar : '#book-tool' ,fit:true,fitColumns : true};
        ...
        $(removeBtn).bind('click', function() {
            ...
            if (rows) {
                ...
                $.messager.confirm('温馨提示', '确认删除此图书?', function(r) {
                	if (!r) {
                        return;
                    }
                    $.ajax({
                        type : "POST",
                        url : '${basePath}manager/book/removeBook.do?id=' + rows.id,
                        async : "false",
        ...
        $(modifyBtn).bind('click', function () {
            ...
            if (row) {
                $('#book-add').dialog({
                    ...
                    cache: false,
                    type : "GET",
                    href: '${basePath}manager/book/modifyBookView.do?bookId=' + row.id,
                    modal: true
                });
            } else {
                layer.msg('必须选择一行!');
...
<div id="book-tool">
	...
	<label style="padding-left: 10px;">书名:</label> <input name="title"
			class="easyui-textbox" style="height: 26px;">
		<label style="padding-left: 10px;">作者:</label> <input name="author"
			class="easyui-textbox" style="height: 26px;"><a
			id="book-searchBtn" href="javascript:void(0)"
			class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
		<!-- <a
			id="book-resetBtn" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-reload" plain="true">重置</a> -->
	</div>
